...
int contB1, modo = 0, contB2, setear = 0, contB3, sube, contB4, baja, x;
...
/*
	...
	//comprobacion de fecha y tiempo
	if(getDate(__DATE__) && getTime(__TIME__)) {
		parse = true;
		//escribir los datos en RTC
		if(RTC.write(tm)) {
			config = true;
		}
	}
...
void __mode__(int &pushMode) {
	if(digitalRead(Mode) == HIGH && contB1 == 0) {
		...
		if(pushMode > 3 && digitalRead(Mode) == HIGH) pushMode = 0;
...
void reloj() {
	...
	if(RTC.read(tm)) {
		...
		print2D(tm.Month);
		dLCD.print("/");
		dLCD.print(tmYearToCalendar(tm.Year));
		delay(1000);
	}
}

bool alarm() {
  if(arrAlarm[0][0] == tm.Hour && arrAlarm[1][0] == tm.Minute && x == 0) return true;
...
//extraccion de datos - __TIME__ Macro
...
//extraccion de datos - __DATE__ Macro
bool getDate(const char *str) {
	char Month[12];
	int d, a;
	int idMes;

	if(sscanf(str, "%s %d %d", Month, &d, &a) != 3) return false;

	for(idMes = 0; idMes < 12; idMes++) {
		if(strcmp(Month, monthName[idMes]) == 0) break;
	}

	if(idMes >= 12) return false;

	tm.Day = d;
	tm.Month = idMes + 1;
	tm.Year = CalendarYrToTm(a);
